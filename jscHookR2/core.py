...
class Callback:
    ...
    def __init__(self, packageName: str, timeOut: int, out_dir: str | None = None, input_dir: str | None = None):
        ...
        if timeOut:
            dump_script += f'setImmediate(function(){{ setTimeout(hook, 10); delay(false, {timeOut});}})'
            repl_script += f'setImmediate(function(){{ setTimeout(hook, 10); delay(false, {timeOut});}})'
        else:
            dump_script += f'setImmediate(function(){{ setTimeout(hook, 10);}})'
            repl_script += f'setImmediate(function(){{ setTimeout(hook, 10);}})'
